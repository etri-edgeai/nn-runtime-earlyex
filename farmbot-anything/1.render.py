            ...
            for rand in range(cfg['1_num_rand']):
                ...
                for view in range(num_views):
                    ...
                    if nonzero_pixels.numel() > 0:
                        ...
                        # category_id = np.array([category_id]).astype(np.int64)
                        key_prefix = f"{num_count}_"
                        # print(f"Saving {key_prefix}...")
                        
                        # verts = mesh.verts_list()[0].cpu().numpy()
                        # faces = mesh.faces_list()[0].cpu().numpy()
                        # txn.put((key_prefix+'verts').encode('utf-8'), verts.tobytes())
                        # txn.put((key_prefix+'faces').encode('utf-8'), faces.tobytes())
                        # if isinstance(mesh.textures, TexturesUV):
                        #     # Serialize and save textures in UV format
                        #     # Note: You need to ensure textures are in a serializable format
                        #     # This is an example and may need to be adjusted based on how your textures are stored
                        #     textures = mesh.textures.maps_padded().cpu().numpy()
                        #     txn.put((key_prefix+'textures').encode('utf-8'), textures.tobytes())
                        # elif isinstance(mesh.textures, TexturesAtlas):
                        #     # Serialize and save textures in Atlas format
                        #     textures_atlas = mesh.textures.atlas_packed().cpu().numpy()
                        #     txn.put((key_prefix+'textures_atlas').encode('utf-8'), textures_atlas.tobytes())
                        ...
                        txn.put((key_prefix+'red_green').encode('utf-8'), np.array(red_green, dtype=np.float32).tobytes())
                        txn.put((key_prefix+'image').encode('utf-8'), rgb_targets[view].cpu().numpy().tobytes())
                        txn.put((key_prefix+'mask').encode('utf-8'), seg_masks[view].cpu().numpy().tobytes())
                        txn.put((key_prefix+'depth').encode('utf-8'), depth_images[view].cpu().numpy().tobytes())
